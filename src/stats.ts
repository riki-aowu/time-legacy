...
export async function collectArchiveStats(): Promise<ArchiveStats> {
  return read(
    ...
    async (tx) => {
      const [blocks, memoryFiles, projectMemories, projects, projectDocs] =
        await Promise.all([
      ...
      for (const block of blocks) {
        ...
        if (type === "text") counts.text += 1;
          ...
          counts.tool_use += 1;
          const raw = block.raw as Row | undefined;
          const name = str(raw?.name) || "未知工具";
          toolNames.set(name, (toolNames.get(name) ?? 0) + 1);
          if (str(raw?.mcp_server_url)) mcpCount += 1;
      ...
      const lastUpdated = [...memoryFiles]
        .map((f) => str(f.updatedAt))
        .filter(Boolean)
        .sort()
        .pop();
      const projectIds = new Set(projects.map((project) => str(project.uuid)));
      const projectsWithMemory = new Set(
        projectMemories
          .map((memory) => str(memory.projectUuid))
          .filter((uuid) => projectIds.has(uuid)),
      ).size;
      return {
        blocks: counts,
        tools: {
          total: counts.tool_use,
          byName: [...toolNames.entries()]
